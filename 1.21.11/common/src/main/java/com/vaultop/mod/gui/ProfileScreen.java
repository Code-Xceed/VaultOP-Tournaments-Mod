...
public class ProfileScreen extends Screen {
    ...
    @Override
    public boolean mouseClicked(net.minecraft.client.gui.Click click, boolean booleanVal) {
        ...
        return super.mouseClicked(click, booleanVal);
    }
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        context.drawCenteredTextWithShadow(this.textRenderer, Text.literal("Player Profile"), this.width / 2, 17, 0xFFFFFFFF);
        ...
        if (profileData == null) {
            context.drawCenteredTextWithShadow(this.textRenderer, Text.literal("§7" + statusText), this.width / 2, this.height / 2, 0xFFFFFFFF);
        } else {
            com.google.gson.JsonObject userObj = profileData.has("user") ? profileData.getAsJsonObject("user") : null;
            ...
            if (avatarTex != null) {
                context.drawTexture(RenderPipelines.GUI_TEXTURED, avatarTex, avatarX, avatarY, 0f, 0f, avatarSize, avatarSize, avatarSize, avatarSize);
            } else {
                ...
                context.drawCenteredTextWithShadow(this.textRenderer, Text.literal(firstChar), avatarX + avatarSize / 2, avatarY + avatarSize / 2 - 4, 0xFFFFFFFF);
            }
            ...
            context.drawTextWithShadow(this.textRenderer, Text.literal("§f" + truncatedName), infoX, infoY, 0xFFFFFFFF);
            ...
            context.drawTextWithShadow(this.textRenderer, Text.literal(ignDisplay), infoX, infoY + 11, 0xFFFFFFFF);
            context.drawTextWithShadow(this.textRenderer, Text.literal("§8Discord link verified"), infoX, infoY + 22, 0xFFFFFFFF);
            ...
            context.drawCenteredTextWithShadow(this.textRenderer, Text.literal(badgeText), leftX + leftW / 2, leftY + identityCardH - 19, 0xFFFFFFFF);
            ...
            context.drawTextWithShadow(this.textRenderer, Text.literal(nameTagText), tagX, tagY, 0xFFFFFFFF);
            ...
            // Drag hint
            context.drawCenteredTextWithShadow(this.textRenderer, Text.literal("§8Drag to rotate"), charCX, centerY + centerH - 14, 0xFFFFFFFF);
            ...
            context.drawTextWithShadow(this.textRenderer, Text.literal("§a⚔ Registered Events §7(" + registeredEvents.size() + ")"), rightX + 12, rightY + 8, 0xFFFFFFFF);
            context.drawTextWithShadow(this.textRenderer, Text.literal("§8Tournament queue and approval status"), rightX + 12, rightY + 19, 0xFFFFFFFF);
            ...
            if (!eventsLoadingStatus.isEmpty()) {
                ...
                for (JsonObject event : registeredEvents) {
                    ...
                    String gameVersion = event.has("gameVersion") ? event.get("gameVersion").getAsString() : "1.21.10";
                    ...
                    if (eventY + getEventCardHeight() >= listTop && eventY <= rightY + rightH - 8) {
                        ...
                        context.drawTextWithShadow(this.textRenderer, Text.literal("§f" + displayName), cardX + 10, eventY + 7, 0xFFFFFFFF);
                        context.drawTextWithShadow(this.textRenderer, Text.literal("§bv" + gameVersion), cardX + 10, eventY + 20, 0xFFFFFFFF);
                        ...
                        context.drawTextWithShadow(this.textRenderer, Text.literal(eventBadgeText), labelX + 5, labelYInside + 2, 0xFFFFFFFF);
                    }
                    eventY += getEventCardStep();
                }
    ...
    private boolean render3DPlayerModel(DrawContext context, Identifier skinTexture, float rotation, int centerX, int centerY, float scale) {
        ...
        try {
            ...
            MatrixStack modelMatrices = new MatrixStack();
            modelMatrices.translate(centerX, centerY - 0.72f * scale, 50.0f);
            modelMatrices.scale(scale, scale, scale);
            modelMatrices.multiply(net.minecraft.util.math.RotationAxis.POSITIVE_Y.rotation((float) Math.toRadians(rotation)));

            // Get vertex consumer for the skin texture
            VertexConsumerProvider.Immediate immediate = MinecraftClient.getInstance().getBufferBuilders().getEntityVertexConsumers();
            VertexConsumer consumer = immediate.getBuffer(net.minecraft.client.render.RenderLayers.entityTranslucent(skinTexture));

            // Render the player model
            int light = 0xF000F0; // full brightness
            int overlay = OverlayTexture.DEFAULT_UV;
            playerModel.render(modelMatrices, consumer, light, overlay, 0xFFFFFFFF);
    ...
    private void drawSkinPart(DrawContext context, Identifier skinTex, int dx, int dy, int dw, int dh, int u, int v, int sw, int sh) {
        context.drawTexture(RenderPipelines.GUI_TEXTURED, skinTex, dx, dy, (float) u, (float) v, dw, dh, sw, sh, 64, 64);
    }
